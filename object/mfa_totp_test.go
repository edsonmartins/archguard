...
func TestTotpMfaInitiate_WithEmptyIssuer(t *testing.T) {
	totpMfa := NewTotpMfaUtil(nil)
	// Test with empty issuer (should default to "ArchGuard")
	...
	// Verify the URL contains the default issuer "ArchGuard"
	if !strings.Contains(mfaProps.URL, "ArchGuard") {
		t.Errorf("URL should contain default issuer 'ArchGuard', got: %s", mfaProps.URL)
	}
}
